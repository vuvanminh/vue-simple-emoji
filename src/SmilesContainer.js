...
export default Vue.component('ls-inputfield', {
  name: 'ls-inputfield',
  directives: {
    onClickaway,
  },
...
<!--v-on-clickaway="hideSmiles"-->
  <div class="smilesCollection" ref="smilesCollection" @click="smilesContainerElementClick">
    <i v-for="smile in smiles" :class="'emoji emoji-' + smile" unselectable="on"></i>
  </div>
  ...
  methods: {
    open(callback, element, orientation) {
      ...
      this.widgetTimeoutId = setTimeout(() => this.close(), 3000);
    },
    close() {
      this.callback = null;
    ...
    smilesContainerElementClick(e) {
      ...
      for (let i = 0; i < e.target.classList.length; i += 1) {
        if (e.target.classList[i].match(/^emoji-/)) {
          smile = e.target.classList[i].substring(6);
          break;
        }
      }
      if (smile) {
        const index = this.categories[0].emojis.indexOf(smile);
        if (index >= 0)
          this.categories[0].emojis.splice(index, 1);

        this.categories[0].emojis.unshift(smile);
        this.categories[0].emojis.length = 54;
        localStorage.setItem('recentEmoji', this.categories[0].emojis);
        this.callback(smile);
      }
    },
    ...
    selectEmojiCategory(category) {
      ...
      if (!this.category) {
        this.categories[0].emojis = localStorage.getItem('recentEmoji');
        if (this.categories[0].emojis)
          this.categories[0].emojis = this.categories[0].emojis.split(',');
        else {
          this.categories[0].emojis = [];
          for (let i = 0; i < 54; i += 1)
            this.categories[0].emojis.push(this.categories[1].emojis[i]);
        }
      }

      this.smiles = this.categories[this.category].emojis;
